...
class Xarxa:
    def __init__(self, learning_rate=0.25):
        ...
        self.pes_oc_ent = np.random.rand(3,
                                         2) * 0.1  # valores generados por funcion np están entre [0,1]; capa oculta  3 nodos y la capa entrada  2 nodos
    ...
    def backward_propagation(self, entradas, valor_deseado):
        ...
        delta_z_oculta = np.dot(self.pesos_sort_oc.T, delta_z_salida) * self.sigmoid_d(
            self.activacion_oculta)  # np.dot producto
